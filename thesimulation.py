...
ITERS = 100 # number of iterations (this is the magic )
 
 
N_FISH = 1000 # number of fish
FISH_BETA = 0.6 # momentum factor of fish [0,1]
FISH_TEMP = 0.5 # softmax temperature of fish [small=min,high=avg]
FISH_SPACING = 3 # target spacing between fish when schooling in length units so steps of 1
FISH_ATTRACTION = 0.1 # strength of the schooling attraction [0,1]
RAND_STRENGTH = 0.05 # strength of the random perturbations 
MAX_FISH_VEL = 3 # maximum speed in length units so steps of 1
FISH_VISION = 3 # defines the acuity of the fish vision
ROTATION = False # strength of fish rotation
 
N_SHARKS = 1 # number of sharks
SHARK_BETA = 0.8 # momentum parameter of sharks
MAX_SHARK_VEL = 5 # maximum velocity of sharks
...
GRACE_PERIOD = 60 # iterations before a shark can start eating (allows dynamics to settle)
...
# normalize function to return the arrays to plot
def get_ocean(i):
...
def fix_world_overflow(coordinates, i):
    ...
    # gets the euclidean distance so its like between the fish and the shark
...
def get_fish_forces(i, fs_dist_vecs, fs_inv_dist_norms):
    ...
    # measure the softmax to follow vector above
...
def run(trials=1):
    ...
    for t in tqdm(range(trials), desc='Percentage of Trials Completed'):
        ...
        # main update loop of the simulation B-)
